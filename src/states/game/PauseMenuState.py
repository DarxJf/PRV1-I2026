...
class PauseMenuState(BaseState):
    ...
    def _show_stats(self) -> None:
        from src.states.game.MenuStatsState import MenuStatsState
        # from src.states.game.StatsMenuState import StatsMenuState
        def open_stats():
            self.state_machine.push(
                MenuStatsState(self.state_machine),
                party=self.play_state.world.party
            )
        
        from src.states.game.FadeInState import FadeInState    
        # 2. Apilamos el fundido hacia negro (opacidad 0 a 255)
        self.state_machine.push(
            FadeInState(self.state_machine),
            color=(0, 0, 0),
            time=0.5,
            on_complete=open_stats
        )
