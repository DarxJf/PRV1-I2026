...
class World:
    def __init__(self, generate_logs: bool = False) -> None:
        ...
        self.powerups: List[Ghost] = []
        self.logs: List[LogPair] = []
        self.logs_spawn_timer: float = 0.0
        self.last_log_y: float = -settings.LOG_HEIGHT + random.randint(0, 80) + 80
        self.log_pair_factory: Factory = Factory(LogPair)
        self.powerup_spawn_timer: float = 0.0
        self.next_powerup_spawn: float = 10.0
        self.ghost_factory: Factory = Factory(Ghost)
